...
class MyLayer(nn.Module):
    def __init__(self,in_features, out_features, entropy_attention,dropout, concat=True):
...
class MotifentropyLayer(nn.Module):
    """
    Simple GAT layer, similar to https://arxiv.org/abs/1710.10903
    """

    def __init__(self, in_features, out_features, dropout, alpha, entropy_vector ,concat=True):
        super(MotifentropyLayer, self).__init__()
        self.dropout = dropout
        self.in_features = in_features
        self.out_features = out_features
        self.alpha = alpha
    ...
    def forward(self, input, adj):
        ...
        a=torch.mm(self.b,self.entropy_vector)
